...
from db import get_person_with_highest_score, get_user_score_and_position
...
while socket_running:
    ...
    while client11.client_ready is False:
        if client11.screen_state == 4:
            for i in range(3):
                username2, score = get_person_with_highest_score(i + 1)
                a = [username2, score]
                leaderboard_list.append(a)
            position2, username2, score = get_user_score_and_position(client11.username)
            curr_user = [position2, username2, score]
            try:
                show_leaderboard_window(leaderboard_window, leaderboard_list, curr_user)
            except:
                pass
    ...
    while round_num < 3:
        ...
        while "11" not in cmd:
            letter_recieved, cmd = handle_data(s)
        if client11.voice is False:
            game_window.enter_word.show()
            game_window.submit_word.show()
            game_window.word_text.show()
        ...
        while cmd != "10":
            ...
            if client11.voice is True:
                ...
                while MyText == client11.curr_word:
                    pass
    ...
    game_window.enter_word.hide()
    game_window.submit_word.hide()
